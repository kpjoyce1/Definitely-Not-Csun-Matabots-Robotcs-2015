...
void shooter_MotorInit (MOTOR_PI* motorA, tMotor title1, tMotor title2, tMotor title3, tSensors sensor);
...
void ReadSpeed(MOTOR_PI* motorA, int time){
  ...
  (*motorA).speedRead = abs((float)(encoder - (*motorA).lastEncoderRead)/((float)time) ) * 9.00; //that is not a unit. You should multiply for a constant. I will leave this way by now.
  (*motorA).lastEncoderRead = encoder; //5.88235*
...
void PI_Control(MOTOR_PI* motorA){
    ...
    //set power in the motor
    	...
    	motor[(*motorA).name3] = (*motorA).controller_output;
		}
		else
		{
			motor[(*motorA).name1] = -127;
    	motor[(*motorA).name2] = -127;
    	motor[(*motorA).name3] = -127;
		}
