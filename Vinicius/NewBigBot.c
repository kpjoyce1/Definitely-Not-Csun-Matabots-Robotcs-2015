...
#define SATURATION_RESPONSE_PERIOD 600.0
...
const float C0 = SAMPLE_PERIOD/(2*SATURATION_RESPONSE_PERIOD);
const float C1 = SAMPLE_PERIOD/(2*SATURATION_RESPONSE_PERIOD);
...
typedef struct {
  ...
  float errorK0output;
  float controller_signal; //controller output to the motor
  ...
  clearTimer(T2);
	while(true)
	{
		...
		if(testStart)
		{
			if(time1[T3] > 100)
			{
				writeDebugStreamLine("%5d,%5d,%5d,%5d,%5d",time1[T4], motorLeftShooter.speedRead,
				motorLeftShooter.controller_signal, motorRightShooter.speedRead, motorRightShooter.controller_signal);
				time1[T3] = 0;
			}
		}

		velocidade_motor_esquerdo=motorLeftShooter.speedRead;
		potencia_motor_esquerdo=motorLeftShooter.controller_signal;
		velocidade_motor_direito=motorRightShooter.speedRead;
		potencia_motor_direito=motorRightShooter.controller_signal;
	}
}
...
void PI_Control(MOTOR_PI* motorA){
    ...
    //make the calcutation for output errors
    errorK1output = (*motorA).errorK0output;
    (*motorA).errorK0output = (*motorA).controller_output - (*motorA).controller_signal;

    //apply control`s law
    if(abs((*motorA).speedRead - (*motorA).speedSet)<5){
    	if((*motorA).proportional) {
    		(*motorA).controller_signal = (*motorA).speedSet;
    		(*motorA).proportional=0;
    	}
			(*motorA).controller_signal = A0*(*motorA).controller_signal+B0*((*motorA).errorK0speed)+B1*errorK1speed;
		}
		...
		//but, if the controller is to far from the point i want to, I want it to go faster. Fast as hell. Now, I would like to play with a proportional
			...
			(*motorA).controller_signal = 10*(1.1111111*((*motorA).speedSet)-(*motorA).speedRead);
			(*motorA).errorK0speed = 0;
			(*motorA).proportional = 1;
		}
		...
		if ((*motorA).speedSet == 0) (*motorA).controller_signal = 0;


    //apply saturation of the controller
    if ( (*motorA).controller_signal > SATURATION_UP ) (*motorA).controller_output = SATURATION_UP;
    else if ( (*motorA).controller_signal < SATURATION_DOWN ) (*motorA).controller_output = SATURATION_DOWN;
    else if ( (*motorA).controller_signal < MOTOR_DEADZONE && (*motorA).controller_signal > -MOTOR_DEADZONE ) (*motorA).controller_output = 0;
		else (*motorA).controller_output = (*motorA).controller_signal;
    ...
    if ( (*motorA).controller_signal < SATURATION_DOWN ) (*motorA).controller_signal = SATURATION_DOWN;
}
void initMotor (MOTOR_PI* motorA, tMotor title1, tMotor title2, tSensors sensor){
		(*motorA).name1 = title1;
		(*motorA).name2 = title2;
