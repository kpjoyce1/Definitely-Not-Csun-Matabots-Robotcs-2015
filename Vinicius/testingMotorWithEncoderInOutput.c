...
int potencia_motor_direito;
int velocidade_motor_direito;
int output;
typedef struct {
...
task main()
{
  ...
  clearTimer(T2);
	while(true)
	{
		drive();
		intake();
		shooter(&motorRightShooter);
		storeTime=time1[T1];
		if(storeTime>=SAMPLE_PERIOD/5){
			ReadSpeed(&motorRightShooter, storeTime);
			clearTimer(T1);
		}
		if(time1[T2]>=SAMPLE_PERIOD){
			ControlFunction(&motorRightShooter);
			clearTimer(T2);
...
void ReadSpeed(MOTOR_PI* motorA, int time){
    ...
    temp =  abs( (float)(encoder - (*motorA).lastEncoderRead) );
    (*motorA).speedRead = abs( 20.0/3.4*(float)(encoder - (*motorA).lastEncoderRead)/((float)time) ); //that is not a unit. You should multiply for a constant. I will leave this way by now.
...
void PI_Control(MOTOR_PI* motorA){
    ...
    //apply saturation on the controller
